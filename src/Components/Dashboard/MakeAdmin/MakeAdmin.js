...
const MakeAdmin = () => {
    ...
    const handleOnSubmit = (e) => {
        ...
        fetch('https://hidden-thicket-41796.herokuapp.com/users/admin', {
            method: 'PUT',
            headers: {
    ...
    const handleCancel = (id) => {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    fetch(`https://hidden-thicket-41796.herokuapp.com/users/admin/${id}`, {
                        method: 'PUT',
                        headers: {
                            'content-type': 'application/json'
                        },
    ...
    return (
        <div>
            <Row className="p-3">
                {isLoading && <div className="sweet-loading text-center vh-100 py-5 my-5">
                ...
                <Col xs md={6}>
                    <Form id="makeAdmin" onSubmit={handleOnSubmit}>
                        ...
                        <Button type="submit" className="my-btn text-black">Make Admin</Button>
                    </Form>
                </Col>
                <Col xs md={6}>
                        ...
                        <tbody>
                            {
                                admins.map(admin =>
                                    <tr key={admin._id}>
                                        ...
                                        <td>{admin.email}</td>
                                        <td>{admin.role}</td>
                                        <td className="delete-btn" style={{ color: 'red' }} onClick={() => handleCancel(admin._id)}>Delete</td>
                                    </tr>
                                )
                            }
                        </tbody>
